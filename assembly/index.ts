...
export function from_gps(lon: f64, lat: f64, maxLat: u32): u64 {
  if (lat < -90 || lat >= 90) {
...
export function to_gps_hex(
  gwCoord: string,
  maxLat: u32,
  maxLon: u32
): string[][] {
  let result = to_gps(<u64>Number.parseInt(gwCoord, 16), maxLat, maxLon);
  return result.map<string[]>((arr: f64[]) => {
    return arr.map<string>((v: f64) => {
      return v.toString();
    });
  });
}

export function to_gps(gwCoord: u64, maxLat: u32, maxLon: u32): f64[][] {
...
export function traverse(
  ...
  let originX: u32 = get_x(gwCoord);
  let originY: u32 = get_y(gwCoord);
  ...
  return make_gw_coord(originX, originY);
}

export function get_x(gwCoord: u64): u32 {
  return u32(gwCoord >> 32);
}

export function get_y(gwCoord: u64): u32 {
  return u32(gwCoord & u32(2 ** 32 - 1));
}

export function make_gw_coord(x: u32, y: u32): u64 {
  return (u64(x) << 32) | y;
}
...
export function pathLength(path: u256): u64 {
  return path.hi2 >> 56;
}

export function hasNextPath(path: u256): bool {
  return pathLength(path) > 0;
}

export function nextDirectionPath(path: u256): DirectionPath {
